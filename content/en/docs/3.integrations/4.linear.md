...
| Assigned to Knecht | The issue is assigned to the account behind the connection. | Ticked by default. Makes "give it to Knecht" a normal assignment in Linear. Knecht taking the issue itself does not fire, see [Triggers](/docs/usage/triggers#knecht-holds-the-issue). |
...
::docs-workflow-yaml{filename="linear-issue-triage.yaml"}
```yaml
version: 1
...
| Source | Fires When | But Only If |
| --- | --- | --- |
| Linear | Status reached: `Todo` | Label is `Bug` |

::docs-workflow-yaml{filename="linear-bug-fix.yaml"}
```yaml
version: 1
name: Linear Bug Fix
...
| Source | Fires When | But Only If |
| --- | --- | --- |
| Linear | Status reached: `Todo` | Label is `Feature` |

::docs-workflow-yaml{filename="linear-plan-feature.yaml"}
```yaml
version: 1
...
::docs-workflow-yaml{filename="linear-assigned-issue.yaml"}
```yaml
version: 1
...
::docs-workflow-yaml{filename="linear-estimate-issue.yaml"}
```yaml
version: 1
