...
steps:
  - type: ddev-start
    id: boot_project
  - type: ai
    prompt: 'Read the ticket of this session. Try to reproduce it on the preview and
      in the code. Do not fix anything. Then apply exactly one label: "bug" if
      something is broken, "enhancement" if it was never built. If the ticket
      lacks the details to decide, apply no label and ask the reporter in a
      comment.'
    id: triage
    label: Triage
```
...
| Source | Fires When | But Only If |
| --- | --- | --- |
| Jira | Label added: `bug` | No conditions |
...
| Source | Fires When | But Only If |
| --- | --- | --- |
| Jira | Label added: `enhancement` | No conditions |
