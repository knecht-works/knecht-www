...
| Assigned to Knecht | The work item is assigned to the account behind the connection. | Ticked by default. Makes "give it to Knecht" a normal assignment in Plane. Knecht [joining the assignees](#knecht-holds-the-work-item) itself does not fire. |
...
::note
Plane creates the label `Bug` with every project, add `Enhancement` under the project settings, "Labels", first. Knecht only applies labels that exist, and a trigger only offers labels that exist. The state "In Review" is not a default either: add it under "States" in the Started group, a state in the Completed group would close the session.
::
...
::docs-workflow-yaml{filename="plane-work-item-triage.yaml"}
```yaml
version: 1
name: Plane Work Item Triage
...
steps:
  ...
  - type: ai
    prompt: >-
      Read the work item of this session. Try to reproduce it in the preview and
      ...
      If the work item lacks the details to decide, apply no label and ask the
...
| Source | Fires When | But Only If |
| --- | --- | --- |
| Plane | State reached: `Todo` | Label is `Bug` |

::docs-workflow-yaml{filename="plane-bug-fix.yaml"}
```yaml
version: 1
name: Plane Bug Fix
...
steps:
  - type: ai
    prompt: >-
      The work item of this session was confirmed as a bug. When you are
      beginning your work move the work item to "In Progress". Then fix it on a
      work branch, verify the fix on the preview, commit in logical batches, and
      open a pull request.

      Comment a short summary of the change on the work item and move it to "In
      Review".
    id: bug_fix
    label: Bug Fix
```
...
| Source | Fires When | But Only If |
| --- | --- | --- |
| Plane | State reached: `Todo` | Label is `Enhancement` |

::docs-workflow-yaml{filename="plane-add-enhancement.yaml"}
```yaml
version: 1
name: Plane Add Enhancement
description: Plan a work item labeled as enhancement and post the plan.
steps:
  - type: ai
    prompt: >-
      The work item of this session is an enhancement.

      Work out how it would be built in this project: the files to touch, the
      steps in order, and what is open.

      Do not change any files or the state. Just post the plan as a comment,
      written for the person who created the work item.
    id: plan_enhancement
...
::note
A work item that a person labels and moves to "Todo" without the triage having seen it starts these workflows without a booted site. Add a boot step to them if that happens in your team.
::
...
::docs-workflow-yaml{filename="plane-assigned-work-item.yaml"}
```yaml
version: 1
name: Plane Assigned Work Item
...
steps:
  - type: ddev-start
    ...
    label: Boot the Site
  - type: ai
    prompt: The work item of this session was assigned to you. Read it with its
      comments, implement it on a work branch, verify it on the preview, commit,
      and open a pull request. Comment a short summary on the work item and move
      it to "In Review".
    id: implement
    label: Implement
```
...
::docs-workflow-yaml{filename="plane-estimate-work-item.yaml"}
```yaml
version: 1
name: Plane Estimate Work Item
description: Estimate a work item in planning and move it to Todo.
steps:
  - type: ai
    prompt: 'Estimate the work item of this session. Find the code it touches and
      judge the effort. Do not change any files. Comment the estimate on the
      work item: a size (S, M, L, or XL), the hours you expect, what drives the
      effort, and what is unclear. Then move the work item to "Todo". If it is
      too vague to estimate, comment your questions and leave it where it is.'
    id: estimate
    label: Estimate
```
::
